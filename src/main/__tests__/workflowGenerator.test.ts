...
function makeNode(id: string, stepType: string, extras: Record<string, unknown> = {}, position = { x: 100, y: 100 }): unknown {
  return {
    id,
    type: "automationStep",
    data: {
...
describe("WorkflowGenerator", () => {
  ...
  describe("node validation", () => {
    it("should keep nodes with valid step types", () => {
      const raw = makeRaw(makeWorkflow({
    ...
    it("should filter out nodes missing id", () => {
      const raw = makeRaw(makeWorkflow({
        nodes: [
          { type: "automationStep", data: { step: { type: "navigate" } }, position: { x: 0, y: 0 } },
          makeNode("2", "click"),
        ],
      }));
      const result = generator.parseAndValidate(raw);
  ...
  describe("node type correction", () => {
    it("should correct node.type to 'browserConditional' for browserConditional steps", () => {
      const raw = makeRaw(makeWorkflow({
        nodes: [makeNode("1", "browserConditional", { browserConditionType: "elementExists", selector: "#el" })],
      }));
      const result = generator.parseAndValidate(raw);
    ...
    it("should correct node.type to 'variableConditional' for variableConditional steps", () => {
      const raw = makeRaw(makeWorkflow({
        nodes: [makeNode("1", "variableConditional", { variableConditionType: "variableEquals", variableName: "x" })],
      }));
      const result = generator.parseAndValidate(raw);
    ...
    it("should set node.type to 'automationStep' for regular steps like navigate", () => {
      const raw = makeRaw(makeWorkflow({
        nodes: [{
          id: "1",
          type: "wrongType",
          data: { step: { id: "1", type: "navigate", description: "Go", value: "https://example.com" } },
          position: { x: 100, y: 100 },
        }],
      }));
      const result = generator.parseAndValidate(raw);
  ...
  describe("autoLayout", () => {
    ...
    it("should space branching nodes horizontally with X_GAP", () => {
      ...
      const level1Nodes = result.nodes.filter(
        (n) => n.id === "2" || n.id === "3"
      );
